            ...
            try:
                ...
                if (anon_user['mac'] == client_mac).any() == False:
                    ...
                    # if user exists, get its anonid and add the new macaddr for the given user
                        ...
                        anonid = list(
                            anon_user[anon_user['name'] == client_name]['anonID'])[0]
                        anon_user = anon_user.append({
                            'anonID': anonid,
                            'name': client_name,
                            'mac': client_mac
                    ...
                    anonid = list(anon_user[anon_user['mac'] == client_mac]['anonID'])[0]
...
if __name__ == '__main__':
    ...
    try:
        ...
        for row in temp_df.itertuples():
            cursor.execute('''INSERT into network_user VALUES(?,?,?,?,?,?,?)''',
                      (int(row[1]), int(te), str(row[3]), str(row[4]), str(row[5]),
                       str(row[6]), str(row[7])))
        db.commit()
